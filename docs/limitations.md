...
- **Public benchmark evidence**: `benchmarks/public-repos.toml` is a curated public-commit suite. The current public evidence table is `benchmarks/results/2026-07-06-public.md` (`67.2%` recall, `50.6%` token precision over `107` scored public cases). Older dated tables are historical only. Treat every table as scoped evidence for those cases, not a leaderboard or broad success claim. The synthetic sample-fixture suite is useful for regression smoke, but it is not currently a release quality gate.
...
- **Learning output**: `agentpack learn` deterministically tracks exposure, structured proof, and derived competency status. Codex or Claude remains the coach and rubric evaluator; AgentPack does not independently verify answer quality, execute submitted verification commands, provide certification, or replace human review.
