...
> **Status: alpha (v0.4.5).** Works, tested, and used in real sessions. Python and JavaScript/TypeScript are the best-supported languages. Current benchmarks are useful regression checks, not broad proof that AgentPack improves coding-agent success. API may change before 1.0.
>
...
`0.4.5` focuses the default workflow on `work`, `finish`, and `doctor`, keeps
`learn` as optional follow-up, makes quickstart output executable commands,
improves route candidate matching, and adds cross-agent continuation guidance
plus route performance coverage.
