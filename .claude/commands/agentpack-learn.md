...
Use AgentPack MCP first. Omit the statement when it is empty, and use global scope only when the user explicitly asks for all projects.
...
Present the returned topics in order with lane, competency, proof requirement, project, `why_now`, exercise, and evidence. Do not invent a fourth topic when history is insufficient. An `unassessed` breadth gap is missing proof, not a demonstrated weakness. Skill confidence is observed exposure, not mastery.
