...
Use when the user invokes `@agentpack-resolve` or `/agentpack-resolve`.

Examples: `/agentpack-resolve pr 123` or `/agentpack-resolve PR #123 focus on backward compatibility`.

The slash/skill invocation owns the full loop; the internal `--check` and `--reply` commands are agent steps, not extra user commands.
